...
- 🚧 **Linked-table DAO validation is still open.** `CreateLinkedTableAsync`, `CreateLinkedOdbcTableAsync`, and `CreateLinkedTextTableAsync` now route Type 4/6 `MSysObjects` rows through catalog-only object-id allocation and catalog-index splicing, but DAO CompactDatabase/OpenRecordset or manual Access UI validation is still tracked in [`writer-disk-format-validation-matrix.md`](writer-disk-format-validation-matrix.md).
...
| **C5** | Route linked-table Type 4/6 `MSysObjects` rows through Access-shaped object-id allocation and catalog-index maintenance. | **Shipped 2026-05-23.** `CreateLinkedTableAsync`, `CreateLinkedOdbcTableAsync`, and `CreateLinkedTextTableAsync` allocate catalog-only object ids and use the MSysObjects splice path. DAO linked-table validation remains in the validation matrix. |
...
- `LinkedTableCatalogWriterTests` verifies that writer-created linked Access/ODBC/text rows allocate nonzero catalog object ids and that inserting a linked table grows the `MSysObjects` index leaves instead of leaving the catalog row unindexed.
