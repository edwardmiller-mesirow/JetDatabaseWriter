...
internal static class AccessRoundTripEnvironment
{
    ...
    /// <summary>Gets the path to a Windows PowerShell host whose bitness matches <see cref="MsAccessPath"/>.</summary>
    ...
    private static ProbeResult DetectCore()
    {
        ...
        // Pick the PowerShell host that matches Office bitness so DAO.DBEngine.120
        // can be activated in-proc.
        ...
        string? powershell;
        if (office32 && File.Exists(SysWow64))
        {
            powershell = SysWow64;
        }
        ...
        return new ProbeResult(msaccess, powershell, null);
    }
