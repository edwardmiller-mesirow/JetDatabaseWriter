...
Fix shipped: `Constants.IndexLeafPage` exposes per-format header offsets, `LeafPageLayout` carries them (`PrefLenOffset`, `PrevPageOffset`, `NextPageOffset`, `TailPageOffset`), and every writer + reader call site (`IndexLeafPageBuilder`, `IndexBTreeBuilder`, `IndexCursor`, `IndexPageCodec`, `IndexBTreeEditor`, and the `IndexMaintainer` path) reads/writes through the layout. See §4.1 for the corrected offset table.
...
- `IndexLeafPageBuilder.TryBuildLeafPage(layout, pageSize, parentTdef, entries)` — thin wrapper that re-emits through `IndexLeafPageBuilder.BuildLeafPage(... enablePrefixCompression: true)` and returns `null` on overflow.
...
- The spliced entry list overflows the leaf payload area (`IndexLeafPageBuilder.TryBuildLeafPage` returns `null`).
...
- `IndexLeafPageBuilder.TryBuildLeafPage(layout, pageSize, parentTdef, entries, prev, next, tail)` — overload that preserves a non-root leaf's sibling-chain header fields. Returns `null` on overflow.
...
3. **W4-C-3 in-place leaf rewrite** — try `IndexLeafPageBuilder.TryBuildLeafPage` with the original leaf's `prev_page` / `next_page` / `tail_page`. On success, compare the new max key to the old max key:
