...
| **C5** | Route linked-table Type 4/6 `MSysObjects` rows through catalog-only object-id allocation and catalog-index maintenance. | **Shipped 2026-05-23.** `CreateLinkedTableAsync`, `CreateLinkedOdbcTableAsync`, and `CreateLinkedTextTableAsync` allocate catalog-only negative object ids, avoid low-24 catalog-id collisions, stamp fixture-aligned flags, non-null placeholder `LvProp`, and linked-object ACE rows, and use the MSysObjects splice path. DAO linked-table validation remains in the validation matrix. |
