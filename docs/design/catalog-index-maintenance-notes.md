...
- `LinkedTableCatalogWriterTests` verifies that writer-created linked Access/ODBC/text rows allocate nonzero catalog object ids, avoid low-24 catalog-id collisions, stamp linked row flags/non-null `LvProp`/ACE rows, and that inserting a linked table grows the `MSysObjects` index leaves instead of leaving the catalog row unindexed.
