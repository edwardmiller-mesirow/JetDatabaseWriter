...
namespace JetDatabaseReader
{
    ...
    /// </summary>
    public sealed class JetDatabaseReader : IDisposable
    {
        ...
        public JetDatabaseReader(string path)
        {
            ...
            int cpId = (sortOrder >> 8) & 0xFF;
            if (cpId == 0) cpId = 1252;  // default to Windows-1252 if unknown
            try { _ansiEncoding = Encoding.GetEncoding(cpId); }
            catch { _ansiEncoding = Encoding.GetEncoding(1252); }
        ...
        private byte[] ReadPageCached(long n)
        {
            ...
            if (_pageCache == null)
                _pageCache = new Dictionary<long, byte[]>();
            ...
            // LRU eviction: if cache is full, remove oldest entry (first in dictionary)
            if (PageCacheSize > 0 && _pageCache.Count >= PageCacheSize)
            {
                long firstKey = _pageCache.Keys.First();
        ...
        public (List<string> Headers,
            ...
            ReadTable(string tableName, int maxRows = 100)
        {
            CatalogEntry entry = GetCatalogEntry(tableName);
        ...
        public IEnumerable<string[]> StreamRows(string tableName, IProgress<int> progress = null)
        {
            CatalogEntry entry = GetCatalogEntry(tableName);
        ...
        /// <summary>
        /// Reads the entire table into a DataTable using streaming (no maxRows limit).
        /// Column names are taken from the TDEF; all cells are typed as System.String.
        /// For multi-million row tables, consider using <see cref="StreamRows"/> instead.
        /// </summary>
        /// <param name="tableName">Table name (case-insensitive). If null or empty, reads the first table.</param>
        /// <param name="progress">Optional progress reporter — receives row count after each page.</param>
        public DataTable ReadTableAsDataTable(string tableName = null, IProgress<int> progress = null)
        {
