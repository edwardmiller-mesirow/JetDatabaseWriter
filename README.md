...
- **Index key fixture parity** — long text/MEMO index keys with embedded line breaks are validated against Access-authored fixtures: Jet4 (V2000 / V2003 / V2007) is byte-exact, and V2010 ACE matches 508 of 510 bytes across fixtures plus DAO-generated probe samples. The final 2-byte ACE suffix model is narrowed to boundary contribution tables but is not yet production-implemented. See [GeneralLegacyEncoderFixtureTests.cs](JetDatabaseWriter.Tests/Indexes/Collation/GeneralLegacyEncoderFixtureTests.cs) and [format-probe notes](docs/format-probe/format-probe-long-row-index-encoding.md).
