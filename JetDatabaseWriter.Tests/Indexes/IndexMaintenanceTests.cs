...
public sealed class IndexMaintenanceTests
{
    ...
    public async Task InsertRow_Single_RebuildsIndexLeaf(DatabaseFormat format)
    {
        ...
        await using (AccessWriter writer = await OpenWriterAsync(stream))
        {
            ...
            await writer.InsertRowAsync("T", [7], this.ct);
        }
    ...
    public async Task AddColumn_PreservesExistingIndex(DatabaseFormat format)
    {
        ...
        await using (AccessWriter writer = await OpenWriterAsync(stream))
        {
            ...
            await writer.InsertRowsAsync("T", [[1], [2]], this.ct);
    ...
    public async Task DropColumn_DropsIndexReferencingDroppedColumn(DatabaseFormat format)
    {
        ...
        await using (AccessWriter writer = await OpenWriterAsync(stream))
        {
            ...
            await writer.InsertRowsAsync("T", [[1, 10]], this.ct);
            await writer.DropColumnAsync("T", "Score", this.ct);
        }
    ...
    public async Task MixedDirectionUniqueCompositeIndex_UpdateDeleteInsert_RoundTrips(DatabaseFormat format)
    {
        ...
        await using (AccessWriter writer = await OpenWriterAsync(stream))
        {
            ...
            int deleted = await writer.DeleteRowsAsync("T", "Id", 1, this.ct);
            Assert.Equal(1, deleted);

            await writer.InsertRowAsync("T", [4, "D", 40], this.ct);
        }
    ...
    public async Task DeleteRows_AfterMultiLevelTreeExists_ShrinksTreeAndStaysConsistent(DatabaseFormat format)
    {
        ...
        await using (AccessWriter writer = await OpenWriterAsync(stream))
        {
            ...
            int deleted = await writer.DeleteRowsAsync("T", "Id", 42, this.ct);
            Assert.Equal(1, deleted);
        }
    ...
    public async Task IncrementalFastPath_SplicesSingleLeaf_OnInsertAndDelete(DatabaseFormat format)
    {
        ...
        await using (AccessWriter writer = await OpenWriterAsync(stream))
        {
            ...
            await writer.InsertRowsAsync("T", [[10], [20], [30]], this.ct);

            // Single insert → incremental splice.
            await writer.InsertRowAsync("T", [25], this.ct);

            // Single delete → incremental splice.
            int deleted = await writer.DeleteRowsAsync("T", "Id", 10, this.ct);
            Assert.Equal(1, deleted);
        }
