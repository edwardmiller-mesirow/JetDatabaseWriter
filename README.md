...
- **Tables created by `CreateTableAsync` are visible in Access and queryable, but `MSysObjects` index maintenance for newly-inserted catalog rows is incomplete.** When the resulting `.accdb` is run through Microsoft Access **Compact & Repair** (or DAO `CompactDatabase`), the composite catalog index `MSysObjects.ParentIdName` is rejected with `Could not find the object 'MSysDb'`. Two pinned tests in [JetDatabaseWriter.Tests/Core/AccessRoundTripTests.cs](JetDatabaseWriter.Tests/Core/AccessRoundTripTests.cs) (`SinglePk_AndSingleColumnFk_SurviveCompactAndRepair`, `CompositePk_AndMultiColumnFk_SurviveCompactAndRepair`) gate the fix. The single-column `MSysObjects.Id` PK splice round-trips correctly. The remaining bug is in the composite-key splice path inside `AccessWriter.TrySpliceCatalogIndexEntryAsync` (sort-key encoding, page-shared prefix recomputation, or row-pointer / entry-start bitmask emission); see the phased plan in [docs/design/catalog-index-maintenance-notes.md](docs/design/catalog-index-maintenance-notes.md) and the leaf-page format reference in [docs/design/index-and-relationship-format-notes.md §4](docs/design/index-and-relationship-format-notes.md).
