...
Local tests already asserted deleted rows are no longer readable through public APIs. The byte-level behavior is now explicit for both modes: default delete/update preserve old row payload bytes and old LVAL pages in the file, while secure erase removes those bytes before LVAL free-list reclamation.
...
- Added Jet4/ACE default-mode coverage for oversized OLE values stored on chained LVAL pages: update appends a fresh chain and leaves the old LVAL pages present; delete leaves both old and current LVAL pages present after the live row is removed.
...
- Added a cross-feature validation matrix for writer-emitted disk-format features: [writer-disk-format-validation-matrix.md](writer-disk-format-validation-matrix.md), with durable DAO validation rules split into [dao-validation-strategy.md](dao-validation-strategy.md).
...
- Updated stale blanket warnings in the index/relationship and complex-column notes to point at the matrix and the phase-specific gaps that existed at the time.
...
New matrix gaps should be promoted into DAO-driven tests when they become high-risk release blockers and a reliable Access-authored fixture can host the mutation.
...
- `AccessWriterOptions.SecureEraseMode = SecureEraseMode.DeletedRowsAndFreedPages` overwrites deleted row bodies and old MEMO/OLE LVAL chains before freeing their pages. The default remains `None`, preserving normal JET logical-delete behavior and backward-compatible remanence by leaving old LVAL pages on disk rather than returning them to the free list.
...
No standalone ESE-inspired backlog remains in this note. Cross-feature validation coverage is summarized in [writer-disk-format-validation-matrix.md](writer-disk-format-validation-matrix.md); add future DAO promotion targets there unless a new ESE-derived risk category appears.
