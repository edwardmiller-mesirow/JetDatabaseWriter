...
- `TryRebuildLeaf(layout, pageSize, parentTdef, entries)` — thin wrapper that re-emits through `IndexLeafPageBuilder.BuildLeafPage(... enablePrefixCompression: true)` and returns `null` on overflow.
