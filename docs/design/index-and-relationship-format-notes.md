...
Validation: 7 round-trip tests in `IndexWriterTests`. This W1 reference predates the broader DAO compact matrix; use [writer-disk-format-validation-matrix.md](writer-disk-format-validation-matrix.md) for the current strongest validation level.
...
- `IndexKeyEncoder.IsColumnTypeSeekable(byte)` — gates resolution (excludes `T_BOOL`; descriptor-scale `T_NUMERIC` participates because `ParentSeekKeyColumn` / `ChildSeekKeyColumn` carry `NumericScale` and the Jet4-vs-ACE numeric twiddle flag from TDEF resolution).
...
- Any key column type is not seekable (currently `T_BOOL`) or its descriptor-aware encoder rejects the concrete value.
...
- **Validation gap (§8):** the format-probe corpus does not contain a `T_BINARY`-keyed index leaf; the byte sequences come from Jackcess. DAO CompactDatabase now covers a representative writer-emitted binary-keyed index, but Access-authored byte comparison remains pending.
...
- `TryEncodeChildSeekKey(ChildSeekIndex, object?[] parentPkValues)` — encodes via the descriptor-aware `IndexKeyEncoder` path; `T_NUMERIC` uses the resolved `NumericScale` and Jet4/ACE numeric form. Returns null on encoder rejection (numeric overflow, text outside General Legacy, etc.).
- `TryReadColumnValuesTypedAsync(RowLocation, TableDef, int[] columnOrdinals, ct)` — single-row typed reader. Returns null if any in-scope column is `T_MEMO` / `T_OLE` / `T_COMPLEX` / `T_ATTACHMENT` because the writer does not currently reassemble LVAL chains or complex child rows for inline access; cascade-delete recursion needs the typed full row to resolve grandchildren. Descriptor-scale `T_NUMERIC` is decoded inline through `JetTypeInfo.ReadFixedTyped`.
...
- **Relationship seek over `T_NUMERIC`.** ✅ **Shipped (2026-05-22).** Parent/child seek-key descriptors now carry `NumericScale` plus the Jet4-vs-ACE numeric encoding flag, `TryEncodeSeekKey` / `TryEncodeChildSeekKey` route `T_NUMERIC` through `EncodeNumericEntryAtDeclaredScale`, and the W22 single-row cascade reader decodes descriptor-scale numeric columns inline. Focused coverage lives in `IndexSeekKeyEncodingTests` and `ForeignKeyEnforcementTests`.
...
Automated DAO coverage now exists for the matrix rows covering the highest-risk PK/FK compact paths, multi-table index/FK stress, storage-maintenance index-page reuse and TDEF-chain reclamation, encrypted compact, fresh ACCDB bootstrap, a representative Northwind-hosted complex-column/LVAL compact path, and representative ACE/Jet4 advanced index key encodings plus B-tree maintenance. Treat remaining §7 validation-gap notes as byte-level parity or per-format residual gaps, not as blanket statements that DAO compact coverage is absent. Jet3 writer-emitted index maintenance remains reader round-trip only until a future MDB compact rig exists. Promote any remaining high-risk matrix row into [DaoValidationTests.cs](../../JetDatabaseWriter.Tests/RoundTrip/DaoValidationTests.cs), [DaoStorageMaintenanceTests.cs](../../JetDatabaseWriter.Tests/RoundTrip/DaoStorageMaintenanceTests.cs), or [AccessRoundTripTests.cs](../../JetDatabaseWriter.Tests/RoundTrip/AccessRoundTripTests.cs).
...
- **W14 `RenameRelationshipAsync` TDEF name cookies.** ✅ **Shipped (W26, 2026-04-27; wide TDEFs, 2026-05-21).** `TryRenameFkLogicalIdxNameAsync` rewrites the per-TDEF logical-idx name cookie on both sides after the catalog row is rewritten; see §7.16. Variable-length name records shrink/grow by shifting the trailing variable-column block, and wide TDEF chains are materialised through the logical-buffer writer when the rename crosses a page boundary. `RelationshipRenameOnMultiPageTDef_SurvivesCompactAndRepair` now runs the renamed FK through DAO CompactDatabase on a Northwind-hosted multi-page child TDEF.
- **Hosted Access validation.** The matrix records DAO tests that pass on Access-equipped hosts, but ordinary CI may still skip them when Access/DAO is unavailable. A Windows runner with Access installed would promote the guarded DAO tests into CI; manual UI checklists remain supplemental evidence only.
