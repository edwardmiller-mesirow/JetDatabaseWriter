...
- **Encryption mutation APIs.** `AccessWriter.EncryptAsync(path, password, AccessEncryptionFormat, …)`, `AccessWriter.DecryptAsync(path, password, …)`, and `AccessWriter.ChangePasswordAsync(path, oldPassword, newPassword, …)` add, remove, or rotate encryption (and switch formats) on an existing file. These APIs accept `ReadOnlyMemory<char>`; pass a mutable `char[]` / `Memory<char>` when the caller needs to erase its own password buffer after the awaited operation completes. Use `AccessWriter.DetectEncryptionFormatAsync(path)` to discover the current format.
