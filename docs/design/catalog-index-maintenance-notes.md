...
- 🚧 **Linked ODBC/text DAO validation is still open.** Linked writers now route Type 4/6 `MSysObjects` rows through catalog-only negative object-id allocation, catalog-index splicing, fixture-aligned row flags, and linked-object ACE rows. Access-file Type 6 links now leave the cache columns null, use DAO-shaped `Database` LVAL storage, and pass DAO CompactDatabase followed by DAO OpenRecordset on the compacted linked table. ODBC/text validation still requires real cached schema metadata in `MSysObjects.LvProp`; that work remains tracked in [`writer-disk-format-validation-matrix.md`](writer-disk-format-validation-matrix.md).
...
| **C5** | Route linked-table Type 4/6 `MSysObjects` rows through catalog-only object-id allocation and catalog-index maintenance. | **Shipped 2026-05-23; expanded 2026-05-24.** `CreateLinkedTableAsync`, `CreateLinkedOdbcTableAsync`, and `CreateLinkedTextTableAsync` allocate catalog-only negative object ids, avoid low-24 catalog-id collisions, stamp fixture-aligned flags and linked-object ACE rows, and use the MSysObjects splice path for Jet4/ACE and Jet3. Access-file `CreateLinkedTableAsync` additionally matches DAO-authored null cache columns and DAO-shaped `Database` LVAL storage, with CompactDatabase plus compacted OpenRecordset coverage. ODBC/text cached-schema validation remains in the validation matrix. |

**C0 + C1 have shipped.** C2, C4, and C5 are now regression-guarded. C3 remains technical debt but is not gating for the covered writer-created table, relationship, complex-column, encrypted-output, and Access-file linked-table flows. DAO Compact & Repair succeeds for those covered flows, while linked ODBC/text cached-schema validation remains tracked separately.
...
- `LinkedTableCatalogWriterTests` verifies that writer-created linked Access/ODBC/text rows allocate catalog-only negative object ids, avoid low-24 catalog-id collisions, stamp linked row flags and linked-object ACE rows, and that inserting a linked table grows the Jet4/ACE `MSysObjects` index leaves instead of leaving the catalog row unindexed. It also asserts Access-file linked rows leave `Lv`/`LvProp`/`LvModule`/`LvExtra` null, survive DAO CompactDatabase, and can be opened through DAO OpenRecordset after compaction. ODBC/text rows still retain placeholder `LvProp` coverage. The suite also asserts splice failures do not leave the just-inserted linked-table catalog row live, copies an Access-authored Jet3 fixture, creates a table, asserts Jet3 `MSysObjects` index leaves gain entries through the format-specific splice path, and stress-inserts catalog rows until fresh `MSysObjects` indexes promote to intermediate roots.
