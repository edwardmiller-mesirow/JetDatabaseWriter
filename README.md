...
- **Index creation: schema + empty leaf, no maintenance ("schema-W1 + W2/W3").** `CreateTableAsync` accepts an optional `IReadOnlyList<IndexDefinition>` that declares logical indexes on the new table.
  - **What is written.** The TDEF index sections (real-index physical descriptor, logical-index entry, logical-index name) are emitted, and one empty B-tree leaf page (`page_type = 0x04`) is appended per index with its page number patched into the matching real-index `first_dp` field.
  - **What is not written.** No B-tree splits, no intermediate (`0x03`) pages, no `tail_page` chain, no prefix compression, and **no insert/update/delete maintenance hooks**. As soon as the table mutates the leaf goes stale until Microsoft Access rebuilds it on the next Compact &amp; Repair pass.
  ...
  - **Supported key column types** (will matter once W5 maintenance lands; today the leaf is empty regardless): `Byte`, `Integer`, `Long Integer`, `Currency`, `Single`, `Double`, `Date/Time`. The internal sort-key encoder rejects `Text`, `GUID`, `Decimal`, `OLE`, `MEMO`, attachment, and complex types with `NotSupportedException`.
  - **Validation gap.** The empty-leaf placeholder has not yet been opened in Microsoft Access on Windows for a Compact &amp; Repair smoke test. Defer production use until that round-trip has been verified by hand.
