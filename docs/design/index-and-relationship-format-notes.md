...
**Status**: text/memo encoding is fully implemented (W7) for General Legacy and General 1033v1 via the Jackcess-derived code tables; fixed-type encodings are implemented in W2 / W12 / W13. The V2010 General long-row suffix is byte-exact for the checked-in Access-authored `Table11` / `Table11_desc` fixture contexts and for the DAO-derived 65-character contribution tables covering the plain, auxiliary, row10, row11, and row12 suffix contexts. Broader arbitrary-byte and binary long-key suffix coverage remains a future compatibility expansion.
...
- **W9b** mutates two TDEFs in place per call. The per-TDEF emission is gated on `_format != Jet3Mdb`; on Jet3, only the W9a `MSysRelationships` rows ship. Either side's TDEF must currently be single-page — multi-page chains throw `NotSupportedException`. As of the multi-page TDEF emission landing, `CreateTableAsync` can produce multi-page TDEFs for wide schemas, so this W9b restriction is now a real user-facing limitation rather than a defensive guard against bytes the writer never emits. `RelationshipWriterTests.CreateRelationshipAsync_MultiPageEndpointTDef_ThrowsBeforeFkLogicalIdxEmission` pins the current failure on both parent-wide and child-wide endpoints and verifies no FK logical-idx entries are emitted. Lifting it is the next follow-up in this area; the `RelationshipManager` in-place mutation helpers (see "cannot mutate the TDEF at page {tdefPage}" sites in [`Relationships/RelationshipManager.cs`](../../JetDatabaseWriter/Relationships/RelationshipManager.cs)) all need to learn the same logical-buffer / page-chain split that `BuildTDefPagesWithIndexOffsets` and `WriteLogicalTDefI32` already implement on the create path. Real-idx sharing per §3.3 reuses any existing slot whose `col_map` matches the relationship's column list exactly.
...
- **W9b multi-page lift design.** Replace the current first-page-only parse/mutate/write helpers with a logical TDEF buffer layer: read the full chain using the same stitching model as `ReadTDefBytesAsync`, parse `FkTDefLayout` from logical offsets, apply append/remove/reclaim/rename shifts against the logical buffer, then split the resized buffer back into physical pages with continuation bodies starting at offset 8. Field writes such as `first_dp`, `used_pages`, `num_idx`, `num_real_idx`, `tdef_len`, and name-record shifts should stay in logical offsets and use `TDefPageBuilder.LogicalToPhysicalTDefOffset` only when materialising pages. Growth may append continuation pages; shrink should either rebuild the chain and free orphaned continuation pages through the existing free-page machinery or deliberately leave unreachable pages documented as leaked until Compact and Repair. The create path must preserve today's ordering guarantees: allocate any new leaf pages before patching `first_dp`, emit both cross-referenced logical entries from one planned snapshot, rebuild the affected real indexes after TDEF materialisation, and rely on `UseTransactionalWrites` or an explicit transaction for atomic rollback if a later write fails. Drop and rename should use the same logical-buffer layer so wide-table create/drop/rename coverage can be promoted together.
