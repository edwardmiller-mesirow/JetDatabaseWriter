...
public sealed class ComplexColumnsFieldsFixtureTests(DatabaseCache db) : IClassFixture<DatabaseCache>
{
    ...
    public async Task ComplexFields_AttachmentColumn_TypeNameIsAttachment()
    {
        ...
        AccessReader reader = await db.GetReaderAsync(TestDatabases.ComplexFields, TestContext.Current.CancellationToken);
        IReadOnlyList<ColumnMetadata> meta = await reader.GetColumnMetadataAsync(DocumentsTable, TestContext.Current.CancellationToken);
        ColumnMetadata? col = meta.FirstOrDefault(c => c.Name.Equals(AttachmentsColumn, StringComparison.OrdinalIgnoreCase));
    ...
    public async Task ComplexFields_Attachment_Row1_CellValueIsNotDbNull()
    {
        ...
        AccessReader reader = await db.GetReaderAsync(TestDatabases.ComplexFields, TestContext.Current.CancellationToken);
        IReadOnlyList<ColumnMetadata> meta = await reader.GetColumnMetadataAsync(DocumentsTable, TestContext.Current.CancellationToken);
        int attachIdx = FindColumnIndex(meta, c => c.Name.Equals(AttachmentsColumn, StringComparison.OrdinalIgnoreCase));
    ...
    public async Task ComplexFields_Attachment_StreamRowsAsStrings_DoesNotThrow()
    {
        ...
        AccessReader reader = await db.GetReaderAsync(TestDatabases.ComplexFields, TestContext.Current.CancellationToken);
        Exception? ex = await Record.ExceptionAsync(async () => await reader.RowsAsStrings(DocumentsTable, cancellationToken: TestContext.Current.CancellationToken).Take(5).ToListAsync(TestContext.Current.CancellationToken));
        Assert.Null(ex);
    }
