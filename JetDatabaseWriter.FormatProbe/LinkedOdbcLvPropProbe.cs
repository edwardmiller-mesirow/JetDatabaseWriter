...
internal static class LinkedOdbcLvPropProbe
{
    ...
    private static string FormatValuePreview(ColumnPropertyEntry entry, DatabaseFormat format)
    {
        ...
        {
            ColumnPropertyBlock.DataTypeBoolean when value.Length >= 1 => value[0] == 0 ? "false" : "true",
            ColumnPropertyBlock.DataTypeByte when value.Length >= 1 => value[0].ToString(CultureInfo.InvariantCulture),
            ColumnPropertyBlock.DataTypeInteger when value.Length >= sizeof(short) => BinaryPrimitives.ReadInt16LittleEndian(value).ToString(CultureInfo.InvariantCulture),
            ColumnPropertyBlock.DataTypeLong when value.Length >= sizeof(int) => BinaryPrimitives.ReadInt32LittleEndian(value).ToString(CultureInfo.InvariantCulture),
            ColumnPropertyBlock.DataTypeSingle when value.Length >= sizeof(float) => BitConverter.Int32BitsToSingle(BinaryPrimitives.ReadInt32LittleEndian(value)).ToString("G9", CultureInfo.InvariantCulture),
            ColumnPropertyBlock.DataTypeDouble when value.Length >= sizeof(double) => BitConverter.Int64BitsToDouble(BinaryPrimitives.ReadInt64LittleEndian(value)).ToString("G17", CultureInfo.InvariantCulture),
            ColumnPropertyBlock.DataTypeDateTime when value.Length >= sizeof(double) => FormatDateTime(value),
            ColumnPropertyBlock.DataTypeText or ColumnPropertyBlock.DataTypeMemo => DecodePropertyText(entry.Value, format),
            ColumnPropertyBlock.DataTypeGuid when value.Length == 16 => new Guid(entry.Value).ToString("D", CultureInfo.InvariantCulture),
            ColumnPropertyBlock.DataTypeBinary when string.Equals(entry.Name, "GUID", StringComparison.OrdinalIgnoreCase) && value.Length == 16 => new Guid(entry.Value).ToString("D", CultureInfo.InvariantCulture),
