...
- **Tables created by `CreateTableAsync` are visible in Access and queryable, but `MSysObjects` index maintenance for newly-inserted catalog rows is incomplete.** When the resulting `.accdb` is run through Microsoft Access **Compact & Repair** (or DAO `CompactDatabase`), composite catalog indexes (e.g. `MSysObjects.ParentIdName`) can be rejected with `Could not find the object 'MSysDb'`. Two pinned tests in [JetDatabaseWriter.Tests/Core/AccessRoundTripTests.cs](JetDatabaseWriter.Tests/Core/AccessRoundTripTests.cs) (`SinglePk_AndSingleColumnFk_SurviveCompactAndRepair`, `CompositePk_AndMultiColumnFk_SurviveCompactAndRepair`) gate the fix. The root cause is that our index leaf reader/writer does not yet model Access's per-entry incremental prefix compression — see [docs/design/index-and-relationship-format-notes.md §4.4.2](docs/design/index-and-relationship-format-notes.md) and the phased plan in [docs/design/catalog-index-maintenance-notes.md](docs/design/catalog-index-maintenance-notes.md) (Phase **C0** is the gating prerequisite for Phase **C1**).
