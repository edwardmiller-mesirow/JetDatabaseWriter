...
**Status:** Phase C0 + C1 shipped, and the later DAO round-trip blockers are closed. All three catalog index fixes landed: **prefix compression cap** (2026-05-03), **entry-start bitmask sentinel** (2026-05-04), and **split-path `maxPrefixLength` cap** (2026-05-04). Later work fixed the separate user-table/FK compact failures by preserving system-table rows on mapped system pages, emitting relationship catalog/ACE metadata, sharing table/index usage-map rows, and reusing single-leaf index pages in place. DAO Compact & Repair now passes for the FK round-trip tests and encrypted compact test on Access-equipped hosts. Updated 2026-05-23: generic index maintenance has grown beyond this note's original C3 wording, C4's zero-slot success case is regression-guarded, C5's linked-table object-id/index/metadata-routing path is shipped, Jet4/ACE catalog callers now throw when the MSysObjects splice path reports `false`, low-level catalog helpers reject duplicate `(ParentId, Name)` rows before splicing, `MSysACEs` / `MSysComplexColumns` inserts now try incremental maintenance before bulk rebuild fallback, Jet4/ACE `MSysObjects` catalog deletes/renames fail fast when incremental maintenance cannot update indexes, and Jet3 linked-table creation now fails fast instead of writing unindexed Type 4/6 catalog rows; C2's non-`MSysObjects` fallback, Jet3 catalog splicing, and linked-table DAO validation remain unresolved. See [`round-trip-openrecordset-hypothesis.md`](round-trip-openrecordset-hypothesis.md) for the closed-out compatibility record and [`writer-disk-format-validation-matrix.md`](writer-disk-format-validation-matrix.md) for the current validation backlog.

## 🚧 Still Unresolved

- 🚧 **C2 is only partially complete.** `InsertSystemRowAndMaintainAsync` now tries incremental maintenance for maintainable system-table inserts, including `MSysComplexColumns` and `MSysACEs`, and Jet4/ACE `MSysObjects` catalog deletes/renames fail fast when incremental maintenance cannot update indexes. Fallback to the full rebuild path remains for other system tables.
- 🚧 **Catalog splicing is not a universal system-table B-tree mutator.** `TrySpliceCatalogIndexEntryAsync` remains Jet4/ACE-only and can still return `false` on malformed or unsupported split/ancestor cases. Jet4/ACE catalog callers now fail fast on that return instead of leaving unmaintained catalog indexes, and Jet3 linked-table creation now throws before writing unindexed Type 4/6 rows, but no proven fallback exists yet and Jet3 catalog splicing remains unresolved.
- 🚧 **Linked-table DAO validation is still open.** Linked writers now route Type 4/6 `MSysObjects` rows through catalog-only negative object-id allocation, catalog-index splicing, fixture-aligned row flags, non-null placeholder `LvProp`, and linked-object ACE rows. DAO validation still requires Access's cached linked-table schema payload in `MSysObjects.LvProp`; that work remains tracked in [`writer-disk-format-validation-matrix.md`](writer-disk-format-validation-matrix.md).
...
- Tries `TryMaintainIndexesIncrementalAsync` for maintainable system tables, including `MSysRelationships`, `MSysComplexColumns`, and `MSysACEs`.
...
| **C2** | Re-route `InsertSystemRowAndMaintainAsync` (used by MSysRelationships / MSysComplexColumns / MSysACEs) away from unsafe full system-table rebuilds wherever possible. | 🚧 **Partially complete, unresolved.** `MSysRelationships`, `MSysComplexColumns`, and `MSysACEs` inserts now try the incremental path when possible, and Jet4/ACE `MSysObjects` catalog deletes/renames fail fast instead of bulk-rebuilding; generic non-`MSysObjects` fallback remains. Non-gating for current DAO compact coverage. |
...
| **C4** | Harden `TryMaintainIndexesIncrementalAsync`'s slot decoder so it no longer silently returns `true` on `slots.Count == 0` for tables known to have indexes. | **Shipped 2026-05-23.** The incremental path now returns `false` with a `C1d no usable real-idx slots` bail reason, so callers can fall back to bulk rebuild. Covered by `FastPath_Bails_WhenIndexedTdefDecodesNoRealIndexKeyColumns`. |
| **C5** | Route linked-table Type 4/6 `MSysObjects` rows through catalog-only object-id allocation and catalog-index maintenance. | **Shipped 2026-05-23.** `CreateLinkedTableAsync`, `CreateLinkedOdbcTableAsync`, and `CreateLinkedTextTableAsync` allocate catalog-only negative object ids, avoid low-24 catalog-id collisions, stamp fixture-aligned flags, non-null placeholder `LvProp`, and linked-object ACE rows, and use the MSysObjects splice path for Jet4/ACE. Jet3 linked-table creation fails fast until Jet3 catalog splicing exists. DAO linked-table validation remains in the validation matrix. |

**C0 + C1 have shipped.** C2/C3 reduce technical debt but are not gating for the covered writer-created table, relationship, complex-column, and encrypted-output flows; C4 and C5 are now regression-guarded. DAO Compact & Repair succeeds for those covered flows, while linked-table DAO validation remains tracked separately.
...
- `LinkedTableCatalogWriterTests` verifies that writer-created linked Access/ODBC/text rows allocate catalog-only negative object ids, avoid low-24 catalog-id collisions, stamp linked row flags, non-null placeholder `LvProp`, and linked-object ACE rows, and that inserting a linked table grows the `MSysObjects` index leaves instead of leaving the catalog row unindexed.
...
4. 🚧 **Jet3 catalog splicing is unresolved.** MSysObjects in Jet3 has different real-idx descriptor sizes and text-key encoding. `TrySpliceCatalogIndexEntryAsync` currently returns `false` for Jet3, and Jet3 linked-table creation now fails fast rather than writing unindexed Type 4/6 rows. Any future Jet3 catalog-index maintenance must branch on `DatabaseFormat`.
