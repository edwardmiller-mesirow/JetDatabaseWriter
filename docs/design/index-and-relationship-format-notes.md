...
- `IndexKeyEncoder.IsColumnTypeSeekable(byte)` — gates resolution (excludes `T_BOOL` and `T_NUMERIC`; the single-call seek-key path only carries a type byte, so it does not yet have the descriptor-scale metadata needed for NUMERIC encoding).
...
- **Single-row cascade reader still skips `T_NUMERIC`.** The W22 single-row reader used by cascade-update / cascade-delete recursion returns null when any in-scope column is `T_NUMERIC`, falling back to the W10 child-table snapshot scan. The shared reader/direct-decoder paths can now decode descriptor-scale NUMERIC values, but this narrow inline helper still lacks a `ColumnInfo`-aware numeric branch.
