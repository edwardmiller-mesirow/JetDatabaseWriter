...
- **Recursive intermediate collapse on cascading delete underflow remains W4-C-8+.** A multi-group delete batch that empties a higher-level intermediate (`newEntries.Count == 0` in `TryStageIntermediateRewritesAsync`) still falls to W4-D. Distinct from the split-side N-way work shipped here; tracked as the trailing W4-C bullet in §9.
