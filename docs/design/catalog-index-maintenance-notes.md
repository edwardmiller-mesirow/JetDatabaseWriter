...
**C0 + C1 have shipped.** C2, C4, and C5 are now regression-guarded. C3 remains technical debt but is not gating for the covered writer-created table, relationship, complex-column, and encrypted-output flows. DAO Compact & Repair succeeds for those covered flows, while Jet3 catalog splicing and linked-table DAO validation remain tracked separately.
