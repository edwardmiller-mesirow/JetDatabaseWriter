...
internal static class FormatProbeApplication
{
    ...
    private static void AddModeValues(List<string> modes, string value)
        => modes.AddRange(value.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries));
    ...
    private static async Task WriteIndexAppendixAsync(string fixturePath, string outPath)
    {
        ...
        _ = sb.AppendLine("# Format probe appendix: indexes, PKs, FKs, relationships");
        _ = sb.AppendLine();
    ...
    private static async Task WriteComplexAppendixAsync(string fixturePath, string outPath)
    {
        ...
        var sb = new StringBuilder();
        _ = sb.AppendLine("# Format probe appendix: complex columns (Attachment, Multi-value)");
        _ = sb.AppendLine();
        _ = sb.AppendLine(CultureInfo.InvariantCulture, $"Source fixture: `JetDatabaseWriter.Tests/Databases/{Path.GetFileName(fixturePath)}`");
        _ = sb.AppendLine(CultureInfo.InvariantCulture, $"Generated by: `JetDatabaseWriter.FormatProbe` on {DateTimeOffset.UtcNow:u}");
        _ = sb.AppendLine(CultureInfo.InvariantCulture, $"Format: {reader.DatabaseFormat}, page size: {reader.PageSize}");
    ...
    private static async Task EmitJet3LeafPagesAsync(AccessReader reader, StringBuilder sb, byte[] td, int numRealIdx)
    {
        ...
        int pos = colStart + (numCols * colDescSz);
        for (int i = 0; i < numCols; i++)
        {
            if (pos >= td.Length)
            {
