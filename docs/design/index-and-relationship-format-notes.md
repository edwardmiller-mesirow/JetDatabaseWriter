...
| **W7** | Text sort-key encoder (General Legacy first; General 1033v1 if a clean spec emerges). | high; defer if possible | ✅ **Partially implemented (2026-04-25).** `IndexKeyEncoder` now handles `T_TEXT (0x0A)` for the documented General Legacy character range only — `0–9 → 0x56–0x5F`, `A–Z`/`a–z → 0x60–0x79` (case-insensitive), terminated by `0x00` (or `0xFF` after the descending ones-complement). Any character outside that range (space, punctuation, non-ASCII, accented Latin, CJK, etc.) throws `NotSupportedException`, which the W5 maintenance loop already swallows to fall through to the existing "leaf goes stale until Compact & Repair" behaviour. The full Access encoding (secondary case/accent weights, locale-specific mappings) is still unspecified in HACKING.md and is **not** implemented here — fail-closed avoids producing a leaf that Microsoft Access could reject as corrupt. 7 unit tests in `IndexKeyEncoderTests` cover empty / digits / case-insensitive letters / digit-vs-letter ordering / descending ones-complement / null / unsupported-character rejection; 2 round-trip tests in `IndexMaintenanceTests` cover an actual text-index rebuild for the supported range and the silent-skip behaviour for an unsupported character. **Not done in W7**: General 1033v1 encoding, secondary weights, non-ASCII, multi-column key concatenation (still blocking multi-column PK / FK rebuild). |
