...
- [x] Add a catalog-bootstrap column descriptor type only if that code changes again.
  ...
  - After: `BootstrapColumnDescriptor`. Do not use `ColumnInfo`; that is parsed/effective `TableDef` metadata, not a bootstrap write-template descriptor.
  - File: `JetDatabaseWriter/Schema/TDefPageBuilder.cs`.
